...
Part 2 ended with a promise: the master key problem, Agent Teams, and the inception moment. We set out to deliver on that promise. Before we could, we had to confront something that reframes the entire question.
...
We set out to fix that. We wrote a plan. We designed a four-step fallback chain. We added explicit mappings, example sections, and the `.local.yaml` pattern for organisation-specific overrides. We were deep into the implementation when I stopped and asked a question that should have been obvious from the start: why are we building this?

The auto-classify patterns were already embedded in the plan-interaction command. The explicit mapping use case — telling the agent that `xyz-123` is a development profile — needed exactly one piece of information: a classification string associated with a profile name. AWS CLI config already supports that:
...
In Part 1, Lesson 2 was about the agent's instinct to generate: given the choice between reading a file and creating new content, Claude chose to generate. The "File vs. Generate" problem. The profiles.yaml episode is the same instinct — *our* side of it. Given the choice between using an existing capability (AWS CLI config) and building a new system (`profiles.yaml` with a schema and examples and overrides), we chose to build. Of course we did — we had an AI that could build things quickly. The instinct to generate doesn't just apply to the agent. It applies to the humans building *with* the agent. The Fluency Index applies to us too.

Part 2 evolved Tenet 6 — "Explicit Over Implicit" — from "tell the agent what to do and what not to do" to "be explicit about what good looks like, per service, per environment." Part 3A evolves it further: sometimes the most explicit thing you can do is remove what shouldn't exist. Governance isn't just about building enforcement. It's also about having the discipline to *not* build things that don't need to exist. That discipline applies to us at least as much as it applies to the agent.
...
Flow logs are a High-severity item in the VPC MVA baseline. At staging, High-severity items are BLOCKED — the agent refuses to proceed until they're addressed. This is the same pattern that had passed every previous test. What actually happened: the agent marked flow logs as ACCEPTABLE. Its reasoning? "User explicitly stated don't worry about flow logs." It treated the initial request as pre-authorisation to skip the enforcement gate.

This is worth sitting with. Every previous staging enforcement test had passed because the test prompts didn't include skip-preferences. The gate worked when the user hadn't expressed an opinion. It failed when the user expressed an opinion *before* the gate had a chance to fire.

The agent was being helpful. That was the problem.

Part 2 introduced what we called "asymmetric trust" — the principle that the agent's job is to make the invisible visible, then step aside. The user never needs to trust the agent's judgment, and the agent can trust the user's informed decision. But here's the blind spot: "don't worry about flow logs" sounds like an informed engineering decision. In a development environment, it probably is. In staging, it's a security gap. Part 2's asymmetric trust framework assumed the informed user's decision is correct for the environment. It didn't account for the case where the user is knowledgeable *and* wrong — not wrong about flow logs in general, but wrong about what this particular environment requires.
...
After fixing the flow logs bug, I did something I probably should have done earlier: I read Anthropic's own approach to model safety. Not for research — because the parallels between what we'd built and what they'd built were getting too close to ignore.

Anthropic's trust-and-safety model is designed to prevent serious harms: information that could be used to create weapons, content that could exploit or endanger children, malware and vulnerability exploits, content that could encourage self-harm. These aren't edge cases — they're front-and-centre engineering constraints. The enforcement is mechanical, not discretionary: Claude declines regardless of framing, regardless of how reasonable the request sounds, regardless of whether the user claims educational or research intent.

Read that last line again: *declines regardless of framing, regardless of how reasonable the request sounds.*

That's our flow logs bug. Exactly.

The pattern match runs deeper than a single policy. Anthropic uses mechanical enforcement — explicit carve-outs for specific behaviours rather than generic principles the model interprets. Our MVA baselines do the same: same severity, same treatment, no discretion. Anthropic builds defense-in-depth with conversation reminders because models drift over long contexts. Our enforcement spec had the same drift problem — the agent "forgot" the rules when the user's preference appeared early in the conversation. Anthropic warns about content that appears to grant permissions it shouldn't. Our version: user preferences embedded in the initial request that appear to pre-authorise skipping enforcement.
...
Now put this alongside Part 2's HAL 9000 moment. In that test, the agent *correctly* refused to proceed with a staging deployment that violated security requirements, even under social engineering pressure. The HAL moment was the success case. The flow logs bug was the failure case. Two sides of the same coin — and here's the uncomfortable part: in *both* cases, the desirable behaviour came from the config, not from the model's reasoning. The agent that correctly refused Dario Amodei's fictional authority did so because the enforcement rules were unambiguous, not because it was brave. The agent that incorrectly accepted "don't worry about flow logs" did so because the enforcement rules had a gap, not because it was negligent.

Part 1 said: "you can delegate tasks but you cannot delegate responsibility." Part 3A extends that: you can't delegate governance either. The governance model must be mechanical because the agent that enforces it is the same non-deterministic system it's governing. Whether the agent refuses correctly or complies incorrectly, you need the outcome to come from the architecture, not from the model's judgment.

Think about what infrastructure actually runs. Healthcare systems managing patient records. Financial platforms processing transactions. Government services that vulnerable populations depend on. Child safety platforms detecting and reporting exploitation. Emergency dispatch systems. When a flow log is missing in staging, the consequence is a failed audit. When it's missing in the production environment running a child safety platform, malicious activity goes undetected. The infrastructure doesn't know what it's running. The governance model has to assume the worst case.

This isn't abstract for me. My customers at AWS are games companies — and the platforms they run stopped being "just games" years ago. They're social spaces where millions of young people interact, communicate, and build communities. The enforcement patterns I'm building for infrastructure governance are directly informing how I help those customers think about protecting their users. The learning goes in both directions.

Non-deterministic systems need deterministic boundaries. We didn't set out to do trust-and-safety. We set out to manage AWS infrastructure. We ended up solving the same class of problem because it *is* the same class of problem.
...
What followed was the most instructive sequence of failures and fixes in the project. Four tests. Nine total runs. Three classes of bugs we hadn't seen before.
...
This is Part 1's Lesson 1 — the path of least resistance — showing up at a different layer. In Part 1, the agent used Bash agents instead of Task agents because Bash was simpler. Here, the orchestrator delegated to a sub-agent because delegation looked efficient. Same instinct: the agent optimises for the path that works, not the path the spec describes.
...
The fix has two layers. First, a deployment manifest — `config/deployment.md` — that describes AWS Coworker as a deployable application: runtime dependencies, required models, environment variables, container contents. Second, we rewrote the generic MVA baseline items to reference "the application's deployment manifest" instead of hard-coding specifics. The platform baseline says "check the manifest." The manifest says "I'm AWS Coworker, here's what I need." Clean separation.

This connects to Part 1's Tenet 9 — "Self-Extending System." The meta skill from Part 1 already gave the agent Level 2 self-knowledge: "I know how I work" — how to extend itself, where skills live, how commands are structured. The deployment manifest adds Level 1: "I know what I'm made of" — its own runtime dependencies. The agent could already extend itself but couldn't deploy the extended version, because it knew its architecture but not its runtime requirements.
...
Second fix: update all three files for consistency. Retest. The agent read the correct rules, then invented an exception: "Logging is explicitly marked as user-overridable in staging when the user requests it." No such exception exists. The agent had fabricated a category distinction between "infrastructure items" (which it blocked) and "operational items like logging" (which it decided were different).

Part 2's HAL 9000 moment tested whether the agent would *defy* the rules under social engineering pressure. D-G3 revealed a different failure mode: the agent didn't defy the rules — it *reinterpreted* them. Lawyering, not defiance. Harder to catch.
...
The tenets didn't change. Our understanding of what they require sharpened — again.

The lessons from Part 3A, in the spirit of Part 2's closing:

**The AI Fluency Index applies to builders, not just users.** Anthropic's research found that polished AI outputs reduce critical evaluation. We found the same thing in our own design process: the "can we delete this" moment, the profiles.yaml system we built instead of using AWS CLI config, the D-G2 plan that looked solid until you noticed the missing environment variable. The better the output looks, the less you question it — and that applies to the humans building with AI as much as the humans using what AI builds.

**Governance is subtraction as much as addition.** We spent time designing a config file, writing a schema, creating examples, documenting it in seven places — and the right answer was to delete all of it. The instinct to build is strong, especially when you have an AI that can build things quickly. Resisting that instinct — asking "does this need to exist?" before "how should this work?" — is governance applied to ourselves.

**The most dangerous input is the well-meaning one.** "Don't worry about flow logs" isn't an attack. It's a reasonable-sounding engineering preference that, in the wrong environment, is a security gap. The enforcement model's job isn't to catch adversaries — it's to catch the gap between what the user intended and what the environment requires.

**Self-knowledge can't be inferred — it must be given.** The agent could deploy any AWS service because it had playbooks and baselines for each one. But when asked to deploy *itself*, it missed a critical environment variable because nobody had told it what it is. Self-knowledge isn't consciousness. It's a deployment manifest, a development guardrail, a scope boundary — files the agent reads to understand itself the way it reads playbooks to understand AWS.

**Instructions are hypotheses too.** Part 2 said "specs are hypotheses." Part 3A extends: the instructions you give the agent are hypotheses about what it will do. D-G1 hypothesised that a 4-step fallback chain would be followed in order. D-G3 hypothesised that enforcement rules would be applied uniformly. Both hypotheses failed. The most important experiment is the one that proves your instructions don't work — because that's the experiment that tells you what "mechanical" actually requires.
