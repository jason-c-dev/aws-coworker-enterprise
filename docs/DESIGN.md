...
**WAR Context Model:** Well-Architected Reviews use different status sets depending on whether they assess a *plan* (what will be built) or *existing infrastructure* (what is there today). Plans use REMEDIATE / ACCEPTABLE / BLOCKED — everything is a gap, the question is what the plan does about it. Reviews use PASS / FAIL — binary assessment of current state. This distinction exists because PASS makes no sense for things that don't exist yet, and REMEDIATE makes no sense for things already deployed. The user can override plan dispositions: remove a planned remediation (if enforcement allows) or request remediation for an acceptable gap. BLOCKED items cannot be overridden at runtime — changing enforcement rules requires modifying `config/environments/environments.yaml`, a tracked git change that provides an additional gate.
