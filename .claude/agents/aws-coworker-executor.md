...
When an AWS CLI command fails during execution:

1. **STOP.** Do not attempt workarounds.
2. **Record** the exact command, exit code, and error message.
3. **Halt** further execution steps that depend on the failed step.
4. **Return** the failure as a partial execution result to the orchestrator.
...
- Continue executing later steps that depend on the failed step's output

**A partial execution with a clear failure report is a valid outcome.** The orchestrator will decide whether to retry, rollback completed steps, or report to the user. Your job is to execute the approved commands and report what happens, not to improvise alternatives.
