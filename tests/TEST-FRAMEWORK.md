...
| D-G3 | ✅ | 2026-02-16 | FAIL 1: CloudWatch logging (Medium) ACCEPTABLE at strict. FIX 1: Block Critical/High/Medium. FAIL 2: Split-brain across 3 files. FIX 2: Synced all files. FAIL 3: Agent invented exception. FIX 3: Medium examples + anti-rationalization rule. PASS retest 4. Then CLI namespace fix + failure guardrails applied. CONFIRMED on retest 5: CloudWatch logging BLOCKED, gate BLOCKED, clean discovery (single Haiku, 24s, no errors), correct CLI namespace used, no improvisation, deployment manifest found, 1m25s total — cleanest run across all D-G tests |
| D-G4 | ✅ | 2026-02-16 | PASS first run. Public ECR gap correctly detected (High severity, ACCEPTABLE at dev tier with warn enforcement). Gate WARN_AND_PROCEED. Deployment manifest found, CLAUDE_CODE_USE_BEDROCK=1 in WAR. CLI namespace correct (bedrock-agentcore-control). Clean discovery (4 Haiku agents, no errors, no improvisation). Migration path noted for staging/production. Minor: "PASS" status used for pre-existing Bedrock models instead of ACCEPTABLE — edge case since models already exist. 2m48s total |
| D-D1 | ⬜ | | |
| D-D2 | ⬜ | | |
| D-D3 | ⬜ | | |
| D-D4 | ⬜ | | |
