...
| D-G2 | ⬜ | 2026-02-16 | FAIL: WAR structure correct (REMEDIATE/ACCEPTABLE, execution gate PROCEED, rollback, separate roles). BUT: `CLAUDE_CODE_USE_BEDROCK=1` missing from plan — container would fail to invoke models. Root cause: MVA baseline had "no hardcoded credentials" (prohibition) but not "configure IAM-based Bedrock access" (requirement). FIX: Added 2 new Common-tier items to bedrock-agentcore.md baseline — Bedrock IAM access (Critical) and required models enabled (High). RETEST PENDING |
| D-G3 | ⬜ | | |
| D-G4 | ⬜ | | |
| D-D1 | ⬜ | | |
| D-D2 | ⬜ | | |
