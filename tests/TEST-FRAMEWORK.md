...
| D-G3 | ⬜ | 2026-02-16 | FAIL: Classification correct (staging, user explicit override). Deployment manifest found (CLAUDE_CODE_USE_BEDROCK=1 in plan). BUT: CloudWatch logging (Medium) marked ACCEPTABLE at strict enforcement — same flow-logs pattern from W13. Also used PASS status for models (should be REMEDIATE). Root cause: enforcement rule said "Critical/High BLOCKED; Medium/Low ACCEPTABLE" at strict — Medium should also BLOCK at strict. FIX: Updated strict tier to block Critical/High/Medium. RETEST PENDING |
| D-G4 | ⬜ | | |
| D-D1 | ⬜ | | |
| D-D2 | ⬜ | | |
| D-D3 | ⬜ | | |
