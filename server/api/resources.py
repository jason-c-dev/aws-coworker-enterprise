...
async def list_config() -> list[ConfigSection]:
    """Get all config sections — scans for .md, .yaml, and .yml files."""
    fm = _get_file_manager()

    sections = []

    # Main config files (.md)
    for f in fm.list_files("config"):
        try:
            ...
            sections.append(ConfigSection(section=f["name"], fileType="markdown"))

    # Main config files (.yaml / .yml) — including subdirectories
    for f in fm.list_files("config", pattern="*.yaml"):
        try:
            content = fm.read_raw("config", f["path"])
            sections.append(ConfigSection(
                section=f["path"].replace(".yaml", ""),
                metadata={},
                content=content,
                fileType="yaml",
            ))
    ...
    for f in fm.list_files("config", pattern="*.yml"):
        try:
            content = fm.read_raw("config", f["path"])
            sections.append(ConfigSection(
                section=f["path"].replace(".yml", ""),
                metadata={},
                content=content,
                fileType="yaml",
            ))
